...
public final class ActivityProfileStateUnitTest {
    ...
    public Object[][] makeActiveProfileResultProvider() {
        ...
        for ( final SimpleInterval loc : Arrays.asList(
            ...
            for ( final double prob : Arrays.asList(0.0, 0.5, 1.0) ) {
                tests.add(new Object[]{ loc, prob, null});
            }
        }
    ...
    public void testActiveProfileResultProvider(SimpleInterval loc, final double prob, ActivityProfileState.Type maybeState) {
        final ActivityProfileState apr = maybeState == null ? new ActivityProfileState(loc, prob) : new ActivityProfileState(loc, prob, maybeState);

        Assert.assertEquals(apr.getLoc(), loc);
        Assert.assertNotNull(apr.toString());
        Assert.assertEquals(apr.getActiveProb(), prob);
        Assert.assertEquals(apr.getType(), maybeState == null ? ActivityProfileState.Type.NONE : maybeState);
    }

    @Test(expectedExceptions = IllegalArgumentException.class)
    public void testError2(){
        final String chr = sequenceDictionary.getSequence(0).getSequenceName();
        new ActivityProfileState(new SimpleInterval(chr, 10, 11), 0.1, null);
    }
}
